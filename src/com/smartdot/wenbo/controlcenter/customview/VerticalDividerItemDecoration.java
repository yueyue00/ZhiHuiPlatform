package com.smartdot.wenbo.controlcenter.customview;
...
public class VerticalDividerItemDecoration extends FlexibleDividerDecoration {
	...
	protected Rect getDividerBound(int position, RecyclerView parent, View child) {
		...
		if (mDividerType == DividerType.DRAWABLE) {
			...
			bounds.left = child.getRight() + params.leftMargin + dividerSize
					/ 2 + transitionX;
	...
	@Override
	protected void setItemOffsets(Rect outRect, int position,
			RecyclerView parent) {
	...
	private int getDividerSize(int position, RecyclerView parent) {
		if (mPaintProvider != null) {
			return (int) mPaintProvider.dividerPaint(position, parent)
					.getStrokeWidth();
		} else if (mSizeProvider != null) {
			return mSizeProvider.dividerSize(position, parent);
		} else if (mDrawableProvider != null) {
			Drawable drawable = mDrawableProvider.drawableProvider(position,
					parent);
	...
	public static class Builder extends
		...
		public Builder marginResId(@DimenRes int topMarginId,
				@DimenRes int bottomMarginId) {
